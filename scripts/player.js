...
let eventsInit = () => {
    $('.player__start').click(e => {
        ...
        if (playerContainer.hasClass('paused')) {
            player.pauseVideo();                              // по клику старт, если видео было на паузе
        } else {
            player.playVideo();                               // по клику пауза, если видео проигрывалось
        }
    });
    ...
    $('.player__volume').click(e => {
        ...
        const newVolPosition = (player.setVolume() / 100) * newButtonPositionVolPercent;

        
        $('.player__playback-button').css({
            right: `${newButtonPositionVolPercent}%`
        });

        player.getVolume(newVolPosition);      
    });
...
function onYouTubeIframeAPIReady() {
    player = new YT.Player('yt-player', {
        ...
        playerVars: {                                           // убираем лишние элементы интерфейса
            controls: 0,
            disablekb: 0,
            showinfo: 0,
            rel: 0,
